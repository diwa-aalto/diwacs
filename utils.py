...
def SnaphotThread(path):
    ...
    base64string = base64.encodestring('%s:%s' % (CAMERA_USER, CAMERA_PASS))
    ...
    try:
        ...
        if win == 5:
            key = OpenKey(HKEY_CURRENT_USER,
                r'Software\Microsoft\Windows\CurrentVersion\Themes\LastTheme')
            ...
            if wallpaper_path.count("%"):
                ...
                wallpaper = os.path.join(os.getenv(env),
                                         wallpaper_path[i2 + 2:])
            else:
...
def ScreenCapture(path, node):
    try:
        ...
        try:
            ...
            pass
        event_id = controller.GetLatestEvent()
        filepath = os.path.join(filepath, str(event_id) + '_' +
                    node + '_' +
...
def CopyToTemp(filepath):
    ...
    try:
        ...
        (fd, temp) = tempfile.mkstemp(dir=temp_path,
                                      suffix=GetFileExtension(filepath))
        os.close(fd)
...
def ArchiveProjectDir(project_id):
    ...
    subprocess.call(['7Zip64/7z', 'a', '-p' + password, '-y',
                     zip_path] + [path])
    for the_file in os.listdir(path):
        ...
        try:
            ...
            logger.exception('Archive project exception: %s', str(e))
...
def ReturnProjectDir(project_id):
    ...
    password = GetProjectPassword(project_id)
    #print "create zip"
    zipfile.ZipFile(zip_path).extractall(path=os.path.join(path, '../'),
                                         pwd=password)
    if os.path.exists(path):
        try:
            os.unlink(zip_path)
        except Exception, e:
            logger.exception('Unzip project dir exception: %s', str(e))
