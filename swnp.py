...
class Node:
    ...
    def __str__(self):
        return "%s: %s with %s screens data %s" % (self.id,self.name,self.screens,self.data)
...
def testIP():
    try:
        ...
        logger.exception("Unable to loop adapters")
        return True 
...
class SWNP:
    ...
    def __init__(self, pgm_group,screens=0,name=None,id=None,context=None,error_handler=None):
        ...
        #Get ip and id
        self.ip = networking.GetLocalIPAddress(STORAGE)
        ...
        #Subscriber threads
        self.sub_thread = threading.Thread(target=self.sub_routine,name="Sub_thread", args=("epgm://"+self.ip+";"+PGM_IP, self.context, ))
        self.sub_thread.daemon=True
        self.sub_thread.start()
        self.sub_thread_sys = threading.Thread(target=self.sub_routine_sys,name="Sub sys thread", args=("epgm://"+self.ip+";"+PGM_IP, self.context, ))
        self.sub_thread_sys.daemon=True
        self.sub_thread_sys.start()
        #join
        #time.sleep(1)
        self.send("SYS", PREFIX_CHOICES[0], "%s_SCREENS_%d_NAME_%s_DATA_%s"%(self.node.id,self.node.screens,self.node.name,self.node.data))
        self.last_joined = self.id
        #print self.node
        #logger.debug("Self.node before addition to node_list %s(%s)",str(self.node),str(type(self.node)))
        ...
        self.ping_thread = threading.Thread(target=self.ping_routine,name="Ping thread",args=(error_handler,))
        self.ping_thread.daemon = True
        self.ping_thread.start()
        self.timeout_stop = threading.Event()
        self.timeout_thread = threading.Thread(target=self.timeout_routine, name="timeout thread")
    ...
    def timeout_routine(self):
        ...
        while not self.timeout_stop.isSet():
            try:
                ...
                #logger.debug('Timeout active computers from db: %s',str(active))
                """nodes = set()
                for row in active:
                    logger.debug("Timeout active row:%s find node %s",str(row),str(row[0]))
                ...
                for node in list(self.NODE_LIST):
                    if not node == self.node and datetime.now()-node.timestamp>timeout:
    ...
    def do_ping(self):
        ...
        try:
            self.send("SYS", PREFIX_CHOICES[4], "%s_SCREENS_%d_NAME_%s_DATA_%s" % (self.id,int(self.node.screens),self.node.name,self.node.data))
        except Exception,e:
            logger.exception('do_ping exception')
    ...
    def sub_routine(self, sub_url, context):
        ...
        while True:
            ...
            try:
                ...
                if msg_obj.PREFIX == 'MSG': 
                    ...
                    pub.sendMessage("message_received", message = msg_obj.PAYLOAD)
            except ValueError:
                pass
            except SystemExit, e:
                break
    ...
    def sub_routine_sys(self, sub_url, context):
        """Subscriber routine for the node ID.
        ...
        while True:
            try:
                # Read envelope with address
                [address,contents] = self.subscriber_sys.recv_multipart()
                msg_obj = json.loads(contents,object_hook=Message.from_json)
    ...
    def close(self):
        ...
        while self.sub_thread_sys.isAlive() and i<limit:
            ...
            i+=1
        i=0
        logger.debug('sub_thread sys closed'+str(self.sub_thread_sys.isAlive()))
        while self.sub_thread.isAlive() and i<limit:
            self.send(self.id,PREFIX_CHOICES[1],self.id)
            time.sleep(1)
            i+=1
        ...
        logger.debug('sub_thread :'+str(self.sub_thread.isAlive())+'sub_thread_sys :'+str(self.sub_thread_sys.isAlive())+'ping_routine :'+str(self.ping_thread.isAlive())+'timeout_thread :'+str(self.timeout_thread.isAlive()))       
    ...
    def send(self,tag,prefix,message):
        ...
        if tag and prefix and message:
            ...
            self.publisher.send_multipart([msg.TAG,json.dumps(msg,default=Message.to_dict)])
    ...
    def sys_handler(self,msg):
        ...
        if msg.PREFIX == 'JOIN':
            ...
            if  payload[0] != self.id:
                ...
                self.send("SYS", PREFIX_CHOICES[4], "%s_SCREENS_%d"%(self.id,int(self.node.screens)))
            #if len(filter(lambda id: id == payload[0], self.NODE_LIST))==0:
            ##print "add to node_list\n" 
            self.NODE_LIST.add(Node(payload[0],int(payload[2]),payload[4],payload[6]))
    ...
    def sync_handler(self,msg): 
        ...
        """
        for id in msg.PAYLOAD.split('_'):
            if len(filter(lambda newid: newid ==id, self.NODE_LIST))==0: 
