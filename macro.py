...
def send_input(type,data,flags,scan=0,mouseData=0):
	Inputs = Input * 1
	if type=='m':
		m = Input_I()
		m.mi = MouseInput(data[0], data[1], mouseData, flags, 0, pointer(extra))
		input = Inputs( (0, m) )
	elif type=='k':
		k = Input_I()
		k.ki = KeyBdInput(data, 0, flags, scan, pointer(extra))
		input = Inputs( (1, k) )
	user32.SendInput(1, pointer(input), sizeof(input[0]))
def key(event,kcode):
