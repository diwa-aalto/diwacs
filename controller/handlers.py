...
class PROJECT_FILE_EVENT_HANDLER(FileSystemEventHandler):
    ...
    def __init__(self, project_id):
        ...
        logger().debug(logmsg, project_id)
    ...
    def on_created(self, event):
        ...
        try:
            ...
            logger().debug(logmsg, str(type(evt_path)), evt_path)
    ...
    def on_modified(self, event):
        ...
        try:
            ...
            file_object = file_object.filter(File.path == event.src_path,
                                             File.project_id == project_id)
            file_object = file_object.order_by(File.id.desc()).first()
            logger().debug('on_modified - query = %s', str(file_object))
            if not file_object:
                file_object = self.on_created(event)
                logger().debug('on_created result: %s', str(file_object))
...
class SCAN_HANDLER(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        try:
            ...
            logger().debug('on_created at: %s (%s)', project_path, basename)
